...
class Agent():
    ...
    def __init__(self, state_size, action_size, random_seed, policy_update_freq=2):
    ...
    def learn(self, experiences, gamma):
        ...
        # delay update until policy frequency
        if self.train_iterations % self.policy_update_freq == 0:
            # ---------------------------- update actor ---------------------------- #
            # Compute actor loss
            actions_pred = self.actor_local(states)
            Q1_pred, Q2_pred = self.critic_local(states, actions_pred)
    ...
    def soft_update(self, local_model, target_model, tau):
        ...
        """
        param_noise = torch.from_numpy(self.noise.sample()).float().to(device)
        
        #add noise to params on update
        for target_param, local_param in zip(target_model.parameters(), local_model.parameters()):
            target_param.data.copy_(tau*(local_param.data+param_noise) + (1.0-tau)*(target_param.data))
